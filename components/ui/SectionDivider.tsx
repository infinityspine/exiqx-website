...
export default function SectionDivider({ align = 'center', label }: SectionDividerProps) {
  ...
  return (
    ...
    >
      <div
        className="h-full bg-gradient-to-r from-red-600/0 via-red-600/80 to-red-600/0"
        style={{
      ...
      {label && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 bg-black">
          <span className="text-xs uppercase tracking-[0.2em] text-red-500/80 font-semibold">
            {label}
          </span>
        </div>
      )}
