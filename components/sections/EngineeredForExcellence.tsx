        ...
        <div className="grid gap-10 md:grid-cols-2 xl:grid-cols-4 items-stretch">
          {features.map((feature, i) => {
            ...
            return (
              <motion.article
                key={feature.title}
                className="group flex h-full flex-col justify-between rounded-3xl border border-red-900/40 bg-gradient-to-b from-zinc-950/98 via-zinc-950/96 to-black/98 px-7 py-8 text-left shadow-[0_18px_45px_rgba(0,0,0,0.6)]"
                initial="hidden"
                whileInView="visible"
              ...
              >
                <div>
                  {/* Icon */}
                  <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-red-900/30">
                ...
                {/* Decorative line */}
                <div className="mt-6 h-[3px] w-16 rounded-full bg-red-700/70 group-hover:w-24 group-hover:bg-red-500 transition-all duration-300" />
              </motion.article>
            )
          })}
        </div>
